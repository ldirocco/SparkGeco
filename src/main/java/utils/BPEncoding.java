...
//        System.out.println(text.length() * 1.0 / S.length());
    ...
    private static byte[] replace(byte[] S, String pair, char c, Int2LongAVLTreeMap counts) {
        ...
        for (int i = 0; i < S.length; i++) {
            ...
            if (c1 == p1 && c2 == p2) {
                ...
                if (i > 0) {
                    c0 = (char) S[i-1];
                    counts.merge(encodePair(c0, c1), (long)-1, Long::sum);
                    counts.merge(encodePair(c0, c), (long)1, Long::sum);
                }
                if (i+1 < S.length) {
                    c3 = (char) S[i+1];
                    counts.merge(encodePair(c2, c3), (long)-1, Long::sum);
                    counts.merge(encodePair(c, c3), (long)1, Long::sum);
                }
    ...
    private static Integer findMaxPair(Int2LongAVLTreeMap counts) {
        ...
        for (Map.Entry<Integer, Long> el: counts.entrySet()) {
